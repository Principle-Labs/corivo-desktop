...
/**
 * shadcn-flavored Sheet built on Radix Dialog — right-side slide-out used
 * by the /now WorkContext detail view. Kept minimal: only the `side="right"`
 * variant we actually use, no animation polish.
 */
...
>(({ className, children, side = "right", ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 flex h-full w-full flex-col gap-4 overflow-y-auto border bg-background p-6 shadow-lg transition ease-in-out sm:max-w-md",
        side === "right" &&
