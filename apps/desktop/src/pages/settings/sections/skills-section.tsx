...
import { SkillsMarketTab } from "./skills-market-tab";

const SOURCE_LABEL: Record<AvailableSkill["source"], string> = {
  agents: "~/.agents/skills/",
  claude: "~/.claude/skills/",
  market: "~/.corivo/skills/market/",
};
...
/**
 * "本地" tab — 列出 host 上扫到的 skill (~/.agents/, ~/.claude/,
 * ~/.corivo/skills/market/)，让用户勾选启用。安装新 skill 走 "市场" tab，
 * 安装后会自动出现在这里 (因为安装目录就是 ~/.corivo/skills/market/，
 * 被 services::skill_share 的 scan 涵盖)。
 */
function LocalSkillsTab() {
  const { t } = useTranslation();
  const { config, update, isSaving } = useConfig();
  const {
    data: available,
    isLoading: scanning,
    refetch,
